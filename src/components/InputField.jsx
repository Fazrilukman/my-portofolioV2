...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    ...
    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(220,38,38,0.4)] border-[#dc2626]"
      : "border-white/20 hover:border-[#dc2626]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#dc2626]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#dc2626] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#dc2626]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
